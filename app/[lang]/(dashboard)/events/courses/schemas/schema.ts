...
export const CourseSchema = z.object({
	...
	name: z.string()
		.min(5, "O nome do curso deve ter pelo menos 5 caracteres."),
	startDate: z
		.string()
		.regex(reg, "Data de início inválida (AAAA-MM-DD)."),
	endDate: z
		.string()
		.regex(reg, "Data de término inválida (AAAA-MM-DD)."),
	registrationDeadline: z
		.string()
		.regex(reg, "Prazo de inscrição inválido (AAAA-MM-DD)."),
	participantLimit: z
		.number()
		.min(1, "O limite de participantes deve ser pelo menos 1."),
	instructors: z
		.string()
		.min(5, "Os instrutores devem ser especificados."),
	description: z
		.string()
