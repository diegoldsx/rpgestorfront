...
import { MessageSchema, MessageSchemaType } from "../schemas/schema";
...
export default function DetailsPage({ searchParams }: PageParams) {
	...
	const { data } = useFetchData(id, (id) => {
		return FAKE_MESSAGES.find((d) => d.id === id);
	});
			...
			<Card className="max-w-5xl mx-auto p-6 shadow-lg rounded-md bg-white">
				<CardHeader>
					<CardTitle className="text-xl font-semibold">
						{data ? "Editar Mensagens" : "Nova Mensagens"}
					</CardTitle>
				</CardHeader>

				<CardContent>
					<GenericForm<MessageSchemaType>
