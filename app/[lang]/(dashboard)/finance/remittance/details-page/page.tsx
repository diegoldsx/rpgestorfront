...
import { fakeRemittance, RemittanceType, RemittanceSchema } from "@/types/Remittance";

export default function DetailsPage({ searchParams }: PageParams) {
	const id = searchParams.id;

	const { data } = useFetchData(id, (id) => {
		return fakeRemittance.find((d) => d.id === id);
	});

	const handleSubmit: SubmitHandler<RemittanceType> = async (data) => {
