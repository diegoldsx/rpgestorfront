...
import { useForm, FormProvider, SubmitHandler, FieldValues, Path, PathValue, DefaultValues } from "react-hook-form";
		...
		<FormProvider {...methods}>
			...
			>
				{columns.map(({ id, title, type, options }) => {
					if (!["text", "select", "checkbox", "date"].includes(type)) return null;
								...
								<Checkbox
									...
									onCheckedChange={(val) => {
										if (typeof val === "boolean") {
											methods.setValue(id as Path<T>, val as PathValue<T, Path<T>>);
										}
									}}
								/>
							);
						...
						case "date":
							fieldComponent = (
								<DatePicker
									...
									onDateChange={(date) =>
										methods.setValue(id as Path<T>, (date ? date.toISOString() : "") as PathValue<T, Path<T>>)
									}
								/>
							);
							break;
					...
					return (
						<FormFieldComponent key={String(id)} name={String(id)} label={title} control={control} errors={errors}>
							{fieldComponent}
						</FormFieldComponent>
					);
				})}
